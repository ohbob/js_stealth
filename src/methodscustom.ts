...
export async function parallel(commands, numConnections = 12) {
...
async function parallelInternal(commands, numConnections = 12) {
  ...
  const waitFunctions: Array<{ wait: () => Promise<any>, index: number, methodName?: string }> = [];
  ...
  for (const { type, cmd, index } of preparedCommands) {
    ...
    if (type === 'method') {
      ...
      const methodName = (fn as any)._methodName || fn.name || 'unknown';
      ...
      const methodName = (cmd as any)?._methodName || (typeof cmd === 'function' ? cmd.name : 'unknown');
