...
import { packUInt32, packUInt16, packUInt8, packInt8, packBool, packString, packDouble, packInt32, unpackUInt32, unpackUInt16, unpackInt16, unpackUInt8, unpackInt8, unpackBool, unpackString, unpackDouble, unpackInt32 } from './core/datatypes.js';
import { getSpellId, METHOD_INDICES, DIRECTIONS } from './constants.js';
class ScriptMethod {
...
export function createMethods(protocol) {
    ...
    const returnObj = {
        ...
        FindTypeEx: async (objType, color, container = 0, inSub = true) => {
            ...
            const method = new ScriptMethod(protocol, METHOD_INDICES.FindTypeEx, [packUInt16, packUInt16, packUInt32, packBool], (buf) => unpackUInt32(buf));
        ...
        GetFindedList: () => {
            ...
            const method = new ScriptMethod(protocol, METHOD_INDICES.GetFindedList, [], (buf) => {
        ...
        GetIgnoreList: () => {
            const method = new ScriptMethod(protocol, METHOD_INDICES.GetIgnoreList, [], (buf) => {
                const count = unpackUInt32(buf, 0);
                const items = [];
                let offset = 4;
                for (let i = 0; i < count; i++) {
